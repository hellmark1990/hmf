...
$('body').on('click', '.toggle', function () {
    ...
    if ($(this).find('> input').is(':checked')) {
        $('input[name="' + $(this).find('> input').data('name') + '"]').val(1);
    } else {
        $('input[name="' + $(this).find('> input').data('name') + '"]').val(0);
    }
});
...
jQuery(function ($) {
    ...
    // Best Readers slider START
    $('.js-best-readers').slick({
        dots: false,
        infinite: true,
        speed: 200,
        slidesToShow: 4,
